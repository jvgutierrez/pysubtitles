...
def fetch_show_list():
    ...
    for link in soup.find_all(href=re.compile(r'^/show/\d+$')):
...
def list_subtitles(show, season, episode, language):
    ...
    for row in soup.find_all('td', class_='NewsTitle', colspan='3'):
        ...
        if row.find_all(title="720/1080"):
            text += " [HD]"
        if row.parent.parent.find_all(title="Hearing Impaired"):
            text += " [HI]"
        ...
        for lang in row.parent.parent.find_all(class_='language'):
            ...
            if language.lower() in lang.text.lower():
                ret.append({'file': text.strip(),
                            'lang': lang.text.strip(),
                            'link': '{0}{1}'.format(BASE_URL, link.strip())})
    return ret
